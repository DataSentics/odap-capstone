...
@dp.transformation(dp.read_table(f"{db_name}.customer_transactions_sdm"))
...
@feature(
    dp.fs.Feature(
        "more_than_two_transactions_last_year_flag",
        "Customer made more than two transactions in the last year",
        fillna_with=False,
    )
)
def more_than_two_transactions_last_year_flag(df):
    return (
        df.groupBy(entity.get_primary_key())
        .agg(f.count("amount").alias("num_transactions"))
        .withColumn(
            "more_than_two_transactions_last_year_flag", f.col("num_transactions") > 2
        )
