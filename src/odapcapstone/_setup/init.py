...
def check_join_customers_and_transactions():
 ...
 ('transaction_city', 'string')]
    ...
    if set(globals()["load_transactions"].result.dtypes) != set(expected_tr_dtypes):
        res_html += fail("the `load_transactions` DataFrame does NOT have the correct schema")
        displayHTML(res_html)
        return
    ...
    if count != 1000:
        res_html += fail(f"the `load_transactions` DataFrame number of rows {count} != 1000")
        displayHTML(res_html)
        return
    ...
    if set(globals()["join_customers_and_transactions"].result.dtypes) != set(expected_joined_dtypes):
        res_html += fail("the `join_customers_and_transactions` DataFrame does NOT have the correct schema")
        displayHTML(res_html)
        return
    
    res_html += checkmark("the `join_customers_and_transactions` DataFrame has the correct schema")
    
    count = globals()["join_customers_and_transactions"].result.count()
    if count != 1000:
        res_html += fail(f"the `join_customers_and_transactions` DataFrame number of rows {count} != 1000")
        displayHTML(res_html)
        return
    
    res_html += checkmark("the `join_customers_and_transactions` DataFrame has 1000 rows")
...
def check_database():
    ...
    try:
        if "SparkSession" not in [arg.annotation.id for arg in ast.parse(inspect.getsource(globals()["create_database"].function)).body[0].args.args]:
    ...
    res_html += checkmark("decorated function SparkSession argument is properly annotated")
...
def check_save_customer_transactions():
 ...
 ('transaction_city', 'string')]
    ...
    if set(globals()["save_customer_transactions"].result.dtypes) != set(expected_joined_dtypes):
        res_html += fail("the `save_customer_transactions` DataFrame does NOT have the correct schema")
        displayHTML(res_html)
        return
    
    res_html += checkmark("the `save_customer_transactions` DataFrame has the correct schema")
    
    count = globals()["save_customer_transactions"].result.count()
    if count != 1000:
        res_html += fail(f"the `save_customer_transactions` DataFrame number of rows {count} != 1000")
        displayHTML(res_html)
        return
    ...
    try:
        df = spark.sql(f"describe table extended {env_db_name}.customer_transactions").toPandas()
    except AnalysisException:
        res_html += fail(f"table `{env_db_name}.customer_transactions` does NOT exist")
        displayHTML(res_html)
        return
    
    res_html += checkmark(f"table `{env_db_name}.customer_transactions` does NOT exists")
    
    if df.loc[20].values[1] != "delta":
        res_html += fail(f"table `{env_db_name}.customer_transactions` is NOT a Delta table")
        displayHTML(res_html)
        return
    ...
    res_html += checkmark(f"table `{env_db_name}.customer_transactions` is in the correct path")
    
    if globals()["save_customer_transactions"].previous_decorator_instance is None or type(globals()["save_customer_transactions"].previous_decorator_instance).__name__ != "table_overwrite":
...
def final_check():
    ...
    if "check_load_customers_pass" not in globals() or not globals()["check_load_customers_pass"]:
    ...
    if "check_join_customers_and_transactions_pass" not in globals() or not globals()["check_join_customers_and_transactions_pass"]:
    ...
    if "check_save_customer_transactions_pass" not in globals() or not globals()["check_save_customer_transactions_pass"]:
...
db_name = "odap_academy_" + ''.join(ch for ch in 'lukas.langr@datasentics.com' if ch.isalnum())
del user
