...
# MAGIC %sql
# MAGIC create database if not exists dev_odap_academy_feature_store
...
def init_targets_table(target_config, db_name, logger: Logger, table_names: TableNames, spark: SparkSession):
    ...
    write = spark.createDataFrame([], schema=targets_table_schema).write.mode("overwrite").option("path", targets_path).format("delta")
...
def init_targets_enum_table(target_config, db_name, logger: Logger, table_names: TableNames, spark: SparkSession):
    ...
    write = spark.createDataFrame([], schema=targets_enum_table_schema).write.mode("overwrite").option("path", targets_enum_path).format("delta")
