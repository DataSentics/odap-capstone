...
@feature(
    dp.fs.Feature(
        "sum_amount_{time_window}", "Sum of amount in last {time_window}", fillna_with=0
    )
)
...
@feature(
    dp.fs.Feature(
        "count_amount_{time_window}",
        "Count of amount in last {time_window}",
        fillna_with=0,
    ),
    dp.fs.Feature(
        "average_amount_more_than_5000_{time_window}",
        "Average of amount is greater than 5000 in last {time_window}",
        fillna_with=False,
    ),
    dp.fs.FeatureWithChange(
        "average_amount_{time_window}",
        "Average of amount in last {time_window}",
        fillna_with=0,
    ),
)
    ...
    def non_agg_features(time_window: str):
        return [
            dp.fs.column(
                ...
                f.col(f"average_amount_{time_window}") > 5000,
            ),
        ]
